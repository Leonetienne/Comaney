...
**AI express creation**: helpers in `budget/express_service.py`, view in `budget/views/express.py`; JSON response only: `{"result":"good","items":[]}` or `{"result":"fail","msg":""}`. The system prompt is assembled from independent feature blocks (`_SMART_CREATE_BASE` + `_SMART_CREATE_PROJECTS`/`_PROJECT_PARTICIPANTS`/`_PROJECT_PAYER`/`_DIRECT_BUDDY`) by `_build_smart_create_system(catalog, blocks=None)`; pass a subset to disable a capability (base is mandatory). Assignment fields (all mutually validated + applied client-side in `express_creation.html`, which presets the buddy section before confirm so the existing confirm path handles them): `project_uid`; project-only `project_participants` (`{name, included, share_percent}` list, `_sanitize_project_participants` → `applyAiParticipants`) and `project_payer` (member name, `_sanitize_project_payer` → `applyAiPayer`); direct-buddy `buddy_name`/`buddy_payer`/`buddy_share_percent` (`_sanitize_direct_buddy` → `applyAiBuddy`; mutually exclusive with `project_uid`). Catalog via `_build_catalog()` (categories, tags, non-archived projects incl. member names, direct buddies). Client name-matching (`nameMatches`) ignores the trailing "(offline ...)" label. On confirm, the success banner's "View expenses" link points to `/buddies/summary/` when every saved expense is a direct buddy expense, to `/projects/<id>/` when they all belong to one project, else the expense list (via `view`/`pid` query params).
