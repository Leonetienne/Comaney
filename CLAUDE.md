...
## Key conventions
- **Achim Archive** (`buddies/`): When an offline member (DummyUser) is removed, their expense history is NOT deleted. Instead it is merged into a special archive dummy called "Achim Archive" (`is_archive=True`). One archive exists per group (`owning_group` set) and one per user (`owning_feuser` set), both created lazily on first removal. Merging: `BuddySpending` rows are reassigned from the deleted dummy to the archive (if archive already participates in the same expense, their `share_percent` is summed); `upfront_payee_dummy` references are updated to point to the archive. Balances and debt simplification stay correct because Achim is a normal group member from the algorithm's point of view. The archive can only be deleted when it holds no expenses. Users can bulk-delete the archive's expenses via a dedicated wipe page with a financial-impact warning; this permanently changes history. Service: `buddies/services/archive.py::BuddyArchiveService`. Views: `kick_dummy` (GET = confirm page, POST = execute), `group_remove_member` (GET for dummies = confirm page, POST with `confirmed=yes` = execute), `personal_archive_wipe`, `group_archive_wipe`. Templates show a one-time Alpine.js modal (`?achim=new` query param) when the archive is first created.
