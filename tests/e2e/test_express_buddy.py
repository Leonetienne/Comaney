...
class TestExpressBuddyPageConfig:
    ...
    def test_buddy_config_absent_without_buddies(self, driver, w, ctx):
        ...
        try:
            ...
            time.sleep(1)
            driver.find_element(By.ID, "id_email").send_keys(c2["email"])
            driver.find_element(By.ID, "id_password").send_keys(c2["password"])
            ...
            driver.get(_url("/login/"))
            time.sleep(1)
            driver.find_element(By.ID, "id_email").send_keys(ctx["email"])
            driver.find_element(By.ID, "id_password").send_keys(ctx["password"])
            driver.find_element(By.CSS_SELECTOR, "button[type=submit]").click()
            time.sleep(2)
