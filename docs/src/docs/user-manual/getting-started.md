...
2. Click the **+** button (bottom-right corner of the page).
3. Fill in a title (e.g. *Coffee*), set the type to **Expense**, and enter the amount.
4. Click **Save**.

That is it. You will see it appear in the expense list.

!!! tip "Got a receipt? Skip the typing."
...
2. Set the type to **Income**.
3. Enter the title (e.g. *January salary*) and the amount.
4. Set a due date matching when the money arrives.
5. Save.
...
2. Enter a name, e.g. *Beach trip 2025* or *Kitchen renovation*.
3. Click **Create**.
...
A solo project is simply a project with you as the only member. It's the default case.
They work like a tag you can archive: group expenses under one name, see the total at a glance, and close it off when the goal is done.

!!! tip "Archive when it is done."
...
1. On the project page, find **Pay someone back**.
2. Select who you are paying. The amount is pre-filled with what you owe.
3. Click **Submit**.
...
The project admin can also clear everything at once with **Settle entire project**. The big advantage: Comaney chains debts together and works out the minimum number of transfers needed, so each person has to make far fewer payments to settle with everyone.
For mathematical reasons, this only works if all settle with each other at once. Because of this, the admin as to invoke it.
...
| I want to... | Go here |
|---|---|
| Set up bills that repeat automatically | [Recurring Expenses](scheduled-expenses.md) |
| Get email reminders before bills are due | [Notifications](notifications.md) |
| Search and filter my expense list | [Search & Filters](dashboard/query-language.md) |
| Customise my dashboard cards | [Dashboard](dashboard/index.md) |
| Split costs with a single friend (no project needed) | [Buddies](buddies/index.md) |
